...
public class Function extends DerivedSymbol implements Identifiable<String> {
	...
	public Function(Context context, ElementInfo element) {
		...
		// super(U.toPlIdentifier(K.FUNCTION_PREFFIX + name), "package",
		// element);
	}
	...
	public String comments() {
		return new QName(getElement().getNamespace(), getElement().getName(),
				getContext().getPrefix(getElement().getNamespace())).toString();
	}
	...
	public String decl(int indent) {
		...
		if (!getParameters().isEmpty()) {
			...
			signature.l(indent + 1, "%s %s %s", getUrlParam().decl(),
					ke.defaultKey(), "'" + getDefaultLocation() + "'");

			signature.l(indent, ")");
		}
		signature.a(indent, String.format("%s %s", ke.returnKey(),
				getReturnType().emit().replaceAll("\\([\\W\\w]+\\)", "")));
	...
	public PreDefinedParameter getUrlParam() {
		...
		do {
			repeated = false;
			url = new PreDefinedParameter(getContext(), name, getContext()
					.getKeywordEmitter().varchar2(), getId());
	...
	public String name() {
		return U.toPlIdentifier(getContext().getSymbolNameEmitter().function(
				getContext().getPrefix(getElement().getNamespace()),
				getElement().getName()));
	}
