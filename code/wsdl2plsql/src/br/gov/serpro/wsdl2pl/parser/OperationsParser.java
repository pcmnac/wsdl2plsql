...
public class OperationsParser {
	...
	public void parse() {
		...
		boolean supportedProtocolFound = false;
		...
		for (Service service : context.getDefs().getServices()) {
			for (Port port : service.getPorts()) {
				...
				if (!supportedProtocolFound
						&& (protocol.equals(K.Protocol.SOAP_1_2) || protocol
								.equals(K.Protocol.SOAP_1_1))) {
					...
					for (Operation operation : portType.getOperations()) {
						BindingOperation bindingOperation = binding
								.getOperation(operation.getName());

						ExtensibilityOperation extensibilityOperation = bindingOperation
								.getOperation();

						String operationStyle = extensibilityOperation
								.getStyle() != null ? extensibilityOperation
						...
						if (operationStyle.equals(K.Style.RPC)) {
							...
							for (BindingElement bindingElement : bindingInput
									.getBindingElements()) {
							...
							ElementInfo elementInfo = new ElementInfo(
									operation.getName(),
									inputSoapBody.getNamespace());
							Function function = new Function(context,
									elementInfo);

							// Input header

							// TODO: remover esse código para usar o resultado
							// do inputSoapHeader
							Part inputHeaderPart = null;// operation.getOutput().getMessage().getPart("params");

							if (inputSoapHeader != null
									|| inputHeaderPart != null) {
								// Part inputHeaderPart =
								// input.getMessage().getPart(inputSoapHeader.getPart());

								Parameter inputHeaderParam = null;

								if (inputHeaderPart.getType() != null) {
									inputHeaderParam = new Parameter(context,
											function, new ElementInfo(
													inputHeaderPart.getName()),
											true);
									inputHeaderParam
											.setType(getTypeDef(inputHeaderPart
													.getType()));
								} else if (inputHeaderPart.getElement() != null) {
									Element inputHeaderElement = context
											.findElement(inputHeaderPart
													.getElement());
									inputHeaderParam = new Parameter(context,
											function, new ElementInfo(
													inputHeaderElement), true);
									inputHeaderParam
											.setType(getTypeDef(inputHeaderElement
													.getType()));
								}
								...
								if (inputHeaderParam != null) {
									...
									throw new RuntimeException(
											"Output header part doesn't have type neither element attributes");
								}
							...
							function.getParameters().addAll(
									extractParameters(inputSoapBody.getParts(),
											function));
							...
							for (BindingElement bindingElement : bindingOutput
									.getBindingElements()) {
								if (bindingElement instanceof AbstractSOAPBody) {
							...
							if (outputSoapHeader != null) {
								Part outputHeaderPart = outputSoapHeader
										.getPart();

								Parameter outputHeaderParam = null;

								if (outputHeaderPart.getType() != null) {
									outputHeaderParam = new Parameter(context,
											function,
											new ElementInfo(outputHeaderPart
													.getName()),
											Parameter.Direction.OUT, true);
									outputHeaderParam
											.setType(getTypeDef(outputHeaderPart
													.getType()));

								} else if (outputHeaderPart.getElement() != null) {
									Element outputHeaderElement = context
											.findElement(outputHeaderPart
													.getElement());
									outputHeaderParam = new Parameter(context,
											function, new ElementInfo(
													outputHeaderElement),
											Parameter.Direction.OUT, true);
									outputHeaderParam
											.setType(getTypeDef(outputHeaderElement
													.getType()));
								}
							...
							if (outputSoapBody.getParts().size() == 1) {
								...
								function.setReturnElement(new ElementInfo(part
										.getName()));
							} else {
								throw new RuntimeException(
										"Output Body message must have one part!");
							}

							function.setDefaultLocation(port.getAddress()
									.getLocation());
							function.setSoapAction(extensibilityOperation
									.getSoapAction());
							...
							if (inputMessage.getParts().size() == 1) {
								Part operationPart = inputMessage.getParts()
										.get(0);

								if (operationPart.getElement() != null) {

									Element element = context
											.findElement(operationPart
													.getElement());
									...
									function.getParameters().addAll(
											extractParameters(inputComplexType,
													function));
									...
									if (outputMessage.getParts().size() != 1) {
										throw new RuntimeException(
												"Output message must have one part!");
									}

									Element outputElement = context
											.findElement(outputMessage
													.getParts().get(0)
													.getElement());
									ComplexType resultComplexType = getComplexType(outputElement);

									Element resultElement = resultComplexType
											.getSequence().getElements().get(0);

									function.setReturnType(getTypeDef(resultElement
											.getType()));
									function.setReturnElement(new ElementInfo(
											resultElement.getName(),
											resultElement.getSchema()
													.getTargetNamespace()));

									function.setDefaultLocation(port
									...
									throw new RuntimeException(
											"document style message part must define \"element\" attribute!");
								}

							} else {
								throw new RuntimeException(
										"Body must have only one <part> element!");
							}
						}
					}
	...
	private List<Parameter> extractParameters(ComplexType message,
			Function function) {
		...
		for (Element element : message.getSequence().getElements()) {
			...
			if (element.getType() != null) {
				...
				Parameter parameter = new Parameter(context, function,
						elementInfo);
	...
	private List<Parameter> extractParameters(List<Part> parts,
			Function function) {
		List<Parameter> parameters = new ArrayList<Parameter>();
		...
		if (element.getEmbeddedType() != null) {
			...
			for (Types types : context.getDefs().getTypes()) {
				for (Schema schema : types.getAllSchemas()) {
					TypeDefinition typeDefinition = schema.getType(element
							.getType());
					if (typeDefinition != null
							&& typeDefinition instanceof ComplexType) {
		...
		if (part.getType() != null) {
			...
			if (complexType.getSequence().getElements().size() != 1) {
				throw new ParsingException(
						"Unknown format: Return complex type should be only one child");
			}

			returnType = getTypeDef(complexType.getSequence().getElements()
					.get(0).getType());
